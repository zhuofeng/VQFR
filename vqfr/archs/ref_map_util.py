...
def feature_match_index(feat_input,
    ...
    for idx in range(0, n_patches, batch_size):
        ...
        if max_idx is None:
            max_idx, max_val = max_idx_tmp.clone(), max_val_tmp.clone()       # [38, 38], [38, 38]
        else:
            indices = max_val_tmp > max_val
            max_val[indices] = max_val_tmp[indices].clone() 
            max_idx[indices] = max_idx_tmp[indices].clone() + idx
    ...
    if norm_input:
        ...
        max_val = max_val.clone() / norm
    
    return max_idx.clone(), max_val.clone() 
