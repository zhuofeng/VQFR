...
from vqfr.ops.upfirdn3d import upfirdn3d
...
def make_resample_kernel3D(k):
    ...
    k = torch.tensor(k, dtype=torch.float32)
    if k.ndim == 1:
        k = k[None, None, :] * k[None, :, None] * k[:, None, None]  # to 3D kernel, outer product
    # normalize
    k /= k.sum()
    return k
...
class UpFirDnSmooth3D(nn.Module):
    """Upsample, FIR filter, and downsample (smooth version).
    Args:
        resample_kernel (list[int]): A list indicating the 1D resample kernel
            magnitude.
    ...
    def __init__(self, resample_kernel, upsample_factor=1, downsample_factor=1, kernel_size=1):
        super(UpFirDnSmooth3D, self).__init__()
        self.upsample_factor = upsample_factor
        self.downsample_factor = downsample_factor
        self.kernel = make_resample_kernel3D(resample_kernel)
        if upsample_factor > 1:
            self.kernel = self.kernel * (upsample_factor**2)
    ...
    def forward(self, x):
        out = upfirdn3d(x, self.kernel.type_as(x), up=1, down=1, pad=self.pad)
        return out
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, resample_kernel=(1, 3, 3, 1)):
        ...
        self.conv1 = ConvLayer(in_channels, in_channels, 3, downsample=False, bias=True, activate=True)
        ...
        self.textlayer = nn.Conv2d(256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
    def forward(self, x):
        out = self.conv1(x)
        out = self.conv2(out)
        skip = self.skip(x)
...
@ARCH_REGISTRY.register()
class StyleGAN23DDiscriminator(nn.Module):
    """StyleGAN2 Discriminator.
    Args:
        out_size (int): The spatial size of outputs.
        channel_multiplier (int): Channel multiplier for large networks of
